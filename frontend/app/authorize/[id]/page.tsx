...
async function fetchSummary(id: string): Promise<{ summary: Summary | null; error: string | null }> {
  try {
    ...
    if (!resp.ok) {
      ...
      return { summary: null, error: typeof body.detail === "string" ? body.detail : "This authorization link is invalid or has expired." };
    }
...
export default async function AuthorizePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <main className="min-h-screen bg-[#f4f6f8] py-8 px-3 sm:px-6">
      <div className="max-w-3xl mx-auto bg-white border border-[#cbd5e1] rounded-lg shadow-sm overflow-hidden font-sans">
        {isAlreadyApproved ? (
          ...
          /* Exact Document Template & Authorization Screen (Matching Screenshots 2 & 3) */
          <AuthorizeForm summary={summary} bookingRef={bookingRef} serviceTitle={serviceTitle} />
        )}
      </div>
    </main>
  );
