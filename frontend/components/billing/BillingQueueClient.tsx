...
export interface BillingLeadRow {
  id: string;
  name: string;
  phone: string;
  email: string;
    ...
    <DataTableCard
      headerContent={
        <TableSearchBar
          ...
          placeholder="Filter billing records by customer, phone, status..."
        ...
        <div className="flex flex-col gap-3 px-4 py-3 sm:flex-row sm:items-center sm:justify-between text-sm text-ink-muted">
          <div className="flex flex-wrap items-center gap-3">
            <div>
              ...
              <span className="font-semibold text-ink">{filteredCount}</span> billing accounts
            </div>
    ...
    >
      <table className="table-modern w-full">
        ...
        <tbody className="divide-y divide-hairline">
          {pagedLeads.length === 0 ? (
            <EmptyTableState
              title={isFiltered ? "No matching billing records" : "No active billing records"}
              subtitle={
                isFiltered
                  ? "Try adjusting your search criteria or clearing filters."
                  : "Leads transferred to billing will automatically appear in this queue."
              }
              ...
              <tr key={lead.id} className="transition-colors hover:bg-surface-raised">
                <td className="px-4 py-3.5">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      ...
                      <p className="font-mono text-[11px] text-ink-faint">ID: {lead.id.slice(0, 8)}...</p>
                    </div>
                  </div>
                </td>
                ...
                <td className="px-4 py-3.5">
                  <div className="inline-flex items-center gap-1.5 rounded-lg border border-hairline bg-surface-raised px-2.5 py-1 text-xs font-semibold capitalize text-ink">
                    ...
                    <span>{lead.service_type || "Standard"}</span>
                  </div>
                </td>
